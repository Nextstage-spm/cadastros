...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && 
    ...
    // Aqui você modifica suas mensagens de sucesso para retornar JSON
    // Por exemplo, onde você tem:
    // $mensagem = "Cadastro realizado com sucesso!";
    // Substitua por:
    echo json_encode(['tipo' => 'success', 'mensagem' => 'Cadastro realizado com sucesso!']);
    exit;
    
    // E onde você tem mensagens de erro:
    // $mensagem = "Erro ao cadastrar.";
    // Substitua por:
    echo json_encode(['tipo' => 'error', 'mensagem' => 'Erro ao cadastrar.']);
    exit;
}
session_start();
...
$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        // No início do arquivo, após o include do config.php
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Verifica se é uma requisição AJAX
...
<head>
    ...
    <title>Sidebar Fixa com Carregamento Dinâmico sem recarregar</title>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav">
            ...
            <hr>
                    ...
                    <a href="#" class="carregar-conteudo" data-url="dic.php"></i> Disciplina</a></li>
...
function inicializarEventos() {
    // Prevenir refresh em todos os formulários
    ...
    // Links de carregamento de conteúdo
...
function carregarConteudo(url) {
    fetch(url)
        ...
        .then(data => {
            ...
            inicializarEventos(); // Reinicializa eventos no novo conteúdo
        })
        .catch(error => {
...
function enviarFormularioAjax(formulario) {
    ...
    .then(data => {
        try {
            ...
            mostrarPopup(resultado.mensagem, resultado.tipo);
            
            if (resultado.tipo === 'success') {
                formulario.reset();
                if (resultado.recarregarTabela) {
                    atualizarListagem();
                }
            }
        } catch (e) {
            ...
            document.getElementById('conteudo').innerHTML = data;
            inicializarEventos(); // Reinicializa eventos após atualizar o conteúdo
        }
    })
    .catch(error => {
        console.error('Erro:', error);
        mostrarPopup('Erro ao processar a requisição.', 'error');
    });
}

// Função para atualizar scripts dinâmicos
...
window.addEventListener('click', function(e) {
    ...
    if (dropdownMenu && !e.target.closest('.user-button')) {

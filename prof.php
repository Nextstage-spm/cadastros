...
<body>
    <div class="container">
        ...
        <form method="POST" action="">
            ...
            <div>
                ...
                <select name="idtitulacao" required>
        ...
        <table>
            ...
            <?php foreach ($result as $row): ?>
                <tr>
                    ...
                    <td>
                        <?php 
                        // Obtém o nome da titulação
                        $titulacaoQuery = $conn->prepare("SELECT nome FROM titulacao WHERE idtitulacao = :idtitulacao");
                        $titulacaoQuery->bindParam(':idtitulacao', $row['idtitulacao']);
                        $titulacaoQuery->execute();
                        $idtitulacao = $titulacaoQuery->fetch(PDO::FETCH_ASSOC);
                        
                        if ($idtitulacao) {
                            echo $idtitulacao['nome'];
                        } else {
                            echo "Titulação não encontrada";
                        }
                        ?>
                    </td>
                    <td class="acao">
                        <a href="#" class="fas fa-edit" onclick="openModal('<?php echo $row['id']; ?>', '<?php echo addslashes($row['nome']); ?>', '<?php echo addslashes($row['email']); ?>', '<?php echo addslashes($row['telefone']); ?>', '<?php echo $row['idtitulacao']; ?>')"></a>
                        <a href="#" class="fas fa-trash-alt" onclick="confirmDelete(<?php echo $row['id']; ?>)"></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
    ...
    <!-- Script e funções JavaScript omitidos para simplificação -->
</body>
</html>

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['nome']) && !empty($_POST['nome']) && 
        isset($_POST['cursos_idd']) && !empty($_POST['cursos_idd']) && 
        isset($_POST['periodo_idd']) && !empty($_POST['periodo_idd']) &&
        isset($_POST['turno_idd']) && !empty($_POST['turno_idd'])) {
        
        // Captura os valores do formulário
        $nome = $_POST['nome'];
        $periodo_idd = $_POST['periodo_idd'];
        $turno_idd = $_POST['turno_idd'];
        $cursos_idd = $_POST['cursos_idd'];

        try {
            // Verifica se a turma já está cadastrada
            $query = $conn->prepare("SELECT COUNT(*) FROM turma WHERE nome = :nome AND periodo_idd = :periodo_idd AND cursos_idd = :cursos_idd AND turno_idd = :turno_idd");
            $query->bindParam(':nome', $nome);
            $query->bindParam(':periodo_idd', $periodo_idd);
            $query->bindParam(':cursos_idd', $cursos_idd);
            $query->bindParam(':turno_idd', $turno_idd);
            ...
            if ($count > 0) {
                ...
                $query = $conn->prepare("INSERT INTO turma (nome, periodo_idd, cursos_cod) VALUES (:nome, :periodo_idd, :cursos_idd)");
...
<br>
            <label for="turrno">Turno</label>
                <select name="turno" id = "select1" requitred>
        ...
        ?>
            ...
            <label for="periodo">Período</label>
                <select name="periodo" id = "select1" requitred>
                   <option value="">Selecione</option>
            <?php
          // Consulta para obter as titulações disponível

...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM titulacao WHERE id = :id");
    $stmt->bindParam(':id', $delete_id);
    
    if ($stmt->execute()) {
        $_SESSION['mensagemSucesso'] = "Registro excluído com sucesso!";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_id'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM titulacao WHERE nome = :nome AND id != :id");
    ...
    if ($count > 0) {
        // Nome já cadastrado
        $_SESSION['mensagemErro'] = "Registro já cadastrado.";
    } else {
        // Preparar e executar a atualização no banco de dados
        $stmt = $conn->prepare("UPDATE titulacao SET nome = :nome WHERE id = :id");
        $stmt->bindParam(':nome', $edit_nome);
        $stmt->bindParam(':id', $edit_id);
...
$sql = "SELECT id, nome FROM titulacao";
...
<head>
    ...
    function editarRegistro(id, nome) {
